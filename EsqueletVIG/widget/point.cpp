...
Point operator* (Point& p, Point& v)
{
	...
	float varx=(p.y*v.z)-(p.z*v.y);
	float vary=(p.z*v.x)-(p.x*v.z);
	float varz=(p.x*v.y)-(p.y*v.x);
	return Point(varx,vary,varz);
}
...
Vector Vector::escalaVec (const Vector v1,const Vector v2,int xyz){


if(xyz==1)x=v1.x*v2.x;
if(xyz==2)y=v1.y*v2.y;
if(xyz==3)z=v1.z*v2.z;
//cout<<"tamaño del vector resultante: "<<x1<<"  "<<y1<<" "<<z1<<endl;
return Vector(x,y,z);
